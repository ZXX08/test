...
#MODIFIED: Add blob-aware media helpers for local-miss fallback.
from ..extras.blob_utils import open_blob_aware_image, prepare_blob_aware_video, should_use_blob
...
if is_pillow_available():
    ...
    # 遥感大图分辨率极高,完全关闭 PIL 解压炸弹检查(数据可信,非攻击)。
    # 设为 None 同时跳过 DecompressionBombWarning 和 DecompressionBombError。
    ...
    def _regularize_images(self, images: list["ImageInput"], **kwargs) -> "RegularizedImageOutput":
        ...
        for image in images:
            if isinstance(image, (str, BinaryIO)):
                #MODIFIED: Read missing local image paths from Azure Blob.
                image = open_blob_aware_image(image)
            elif isinstance(image, bytes):
                #MODIFIED: Keep image decoding through the blob-aware helper.
                image = open_blob_aware_image(image)
            elif isinstance(image, dict):
                if image["bytes"] is not None:
                    #MODIFIED: Keep encoded image decoding through the blob-aware helper.
                    image = open_blob_aware_image(image["bytes"])
                else:
                    #MODIFIED: Read missing local image paths from Azure Blob.
                    image = open_blob_aware_image(image["path"])
    ...
    def _regularize_videos(self, videos: list["VideoInput"], **kwargs) -> "RegularizedVideoOutput":
        ...
        for video in videos:
            ...
            if _check_video_is_nested_images(video):
                for frame in video:
                    #MODIFIED: Allow missing local frame paths to be loaded from Azure Blob later.
                    if (
                        not is_valid_image(frame)
                        and not isinstance(frame, dict)
                        and not os.path.exists(frame)
                        and not should_use_blob(frame)
                    ):
                        raise ValueError("Invalid image found in video frames.")
                frames = video
                durations.append(len(frames) / kwargs.get("video_fps", 2.0))
            else:
                #MODIFIED: Cache missing local video paths from Azure Blob before PyAV opens them.
                video = prepare_blob_aware_video(video)
